...
let RAW_URL = TARGET_URLS ? TARGET_URLS.split(',').map(u => u.trim()).filter(Boolean)[0] : 'https://example.com';

// 🔥 NO-VIDEO MODE LOGIC (The "!" Check)
let isNoVideoMode = false;
let TARGET_URL = RAW_URL;

if (RAW_URL.startsWith('!')) {
    isNoVideoMode = true;
    TARGET_URL = RAW_URL.substring(1); // '!' ko hata kar baaki URL use karega
    console.log(`[💡] NO-VIDEO MODE DETECTED! Centered Text Overlay will be shown.`);
}
    ...
    try {
        await obs.call('CreateInput', {
            ...
            inputSettings: {
                ...
                font: { face: 'Liberation Sans', size: 64, flags: 1 }, // Bada font Center ke liye
                color1: 0xFFFFFFFF, outline: true, outline_size: 4, outline_color: 0x000000FF,
                align: 'center', valign: 'center'
            }, 
            sceneItemEnabled: isNoVideoMode // 🔥 TEXT SIRF TAB ON HOGA JAB URL MEIN "!" HOGA
        });
...
async function startOBS() {
    ...
    console.log('[*] Starting OBS Engine with --startstreaming...');
