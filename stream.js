...
//     // stream.js ke top par (jahan baqi requires hain)
...
// // Current working directory mein hamara download kiya hua extension folder
// const extensionPath = path.join(process.cwd(), 'allow-cors-ext');
...
//     // 👇 YAHAN EXTENSION LOAD KARNE KE ARGS 👇
...
// ============== ALhamdullah pehly yeh thaa 1st prioty , upper yeh audio804 k volume ko control karne k try karty hai ok ==========================
...
// const selectedQuality = process.env.STREAM_QUALITY || 'Original (1080p Max)';
// const selectedFormat = process.env.STREAM_FORMAT || 'Original (16:9 Standard)'; 
// const ENABLE_BLACK_OVERLAY = process.env.ENABLE_BLACK_OVERLAY === 'ON';
...
//                     const junkClasses = '.chat, #chat, header, footer, .sidebar, .banner, .ads, [class*="overlay"]:not(#smart-stream-overlay):not(#sport4u-watermark):not(#sport4u-black-overlay), [id*="pop"], [class*="pop"], a[href*="extension"], [class*="notification"], [id*="notification"]';
//                     document.querySelectorAll(junkClasses).forEach(el => { 
//                         try { el.remove(); } catch(e){ el.style.setProperty('display', 'none', 'important'); } 
//                     });
...
//                     const adKeywords = ['jerk', 'mate', 'free', 'online', 'adult', 'dating', 'close', 'notification', 'justine', 'paying', 'job'];
//                     document.querySelectorAll('div, section, span, a').forEach(el => {
//                         if (el.id === 'smart-stream-overlay' || el.id === 'sport4u-watermark' || el.id === 'sport4u-black-overlay') return;
...
//                                 document.querySelectorAll('video, audio').forEach(m => { m.muted = !isAudioEnabled; m.volume = isAudioEnabled ? 1.0 : 0.0; }); 
//                                 if (isAudioEnabled) {
//                                     document.querySelectorAll('.jw-icon-volume.jw-off, .vjs-vol-muted, .plyr__control--pressed[data-plyr="mute"]').forEach(btn => { try { btn.click(); } catch(e){} });
//                                 }
...
//             console.log(`[🎶] Starting Audio in INFINITE LOOP...`);
//             audioProcess = spawn('ffplay', ['-nodisp', '-loop', '0', '-loglevel', 'quiet', foundAudioPath]);
//             audioProcess.on('error', (err) => console.log(`[!] Audio Process Error: ${err.message}`));
//         } else {
...
//     // START PHASE TIMER AFTER INITIALIZATION
//     phaseEndTime = phases[currentPhaseIndex].durationMs ? Date.now() + phases[currentPhaseIndex].durationMs : null;
        ...
        await page.evaluate(() => {
            setInterval(() => {
                try {
                    ...
                    const adKeywords = ['jerk', 'mate', 'free', 'online', 'adult', 'dating', 'close', 'notification', 'justine', 'paying', 'job'];
                    document.querySelectorAll('div, section, span, a').forEach(el => {
                        if (el.id === 'smart-stream-overlay' || el.id === 'sport4u-watermark' || el.id === 'sport4u-black-overlay' || el.id === 'sport4u-random-pic') return;
...
// ================ Alhamdullah fixed done, iss mee fixed size text overlay alhamdullah women aisa cup india =================
...
//             const cmd = `gh workflow run main.yml -f target_urls="${targetUrls}" -f youtube_stream_key="${YT_KEY}" -f facebook_stream_key="${FB_KEY}" -f stream_format="${format}" -f stream_quality="${quality}" -f server_selection="${server}" -f proxy_engine="${PROXY_ENGINE}" -f enable_black_overlay="${blackOverlayStatus}" -f enable_stream_audio="${streamAudioStatus}" -f enable_background_audio="${bgAudioStatus}" -f enable_pic_overlay="${picOverlayStatus}" -f background_audio_volume="${bgAudioVolumeStatus}" -f custom_duration="None"`;
//             execSync(cmd, { stdio: 'inherit' });
//             setTimeout(async () => {
//                 await cleanup(); 
//                 process.exit(0); 
...
// ============== alhamdullah aduio vloume control work ==================
...
//             const cmd = `gh workflow run main.yml -f target_urls="${targetUrls}" -f youtube_stream_key="${YT_KEY}" -f facebook_stream_key="${FB_KEY}" -f stream_format="${format}" -f stream_quality="${quality}" -f server_selection="${server}" -f proxy_engine="${PROXY_ENGINE}" -f enable_black_overlay="${blackOverlayStatus}" -f enable_stream_audio="${streamAudioStatus}" -f enable_background_audio="${bgAudioStatus}" -f background_audio_volume="${bgAudioVolumeStatus}" -f custom_duration="None"`;
//             execSync(cmd, { stdio: 'inherit' });
//             setTimeout(async () => {
//                 await cleanup(); 
//                 process.exit(0); 
...
// ============== *** 1 : ALhamdullah pehly yeh thaa 1st prioty , upper yeh audio804 k volume ko control karne k try karty hai ok ==========================
...
//                         // test2
//                         // overlay.innerHTML = 'Check Comment Today Top  Matches Real Madrid , chelse , manunity , leeds etc   👉<br><span style="color: #ff4d4d; font-size: 24px; line-height: 1.2;"> Live on sport4u.online</span>';
//                         overlay.innerHTML = 'DFB Pokal,Italy-Coppa-Italia . Please Watch here on Google👉<br><span style="color: #ff4d4d; font-size: 24px; line-height: 1.2;">sport4u.online</span>';
...
// =============== done yaha taak bas upper eek new system add karty hai multiple live matcehs facebook ko eek hey action ================
...
// const selectedFormat = process.env.STREAM_FORMAT || 'Original (16:9 Standard)'; 
// const ENABLE_BLACK_OVERLAY = process.env.ENABLE_BLACK_OVERLAY === 'ON';
// const ENABLE_STREAM_AUDIO = process.env.ENABLE_STREAM_AUDIO !== 'OFF'; // 🔥 MAIN PLAYER AUDIO
// const ENABLE_BACKGROUND_AUDIO = process.env.ENABLE_BACKGROUND_AUDIO === 'ON'; 
...
//                         // Left border (0%) - Hidden
//                         const leftBlock = document.createElement('div');
//                         leftBlock.style.cssText = `position: absolute !important; top: 0 !important; left: 0 !important; width: 0% !important; height: 100% !important; background-color: #000000 !important;`;
                        
//                         // Right border (0%) - Hidden
//                         const rightBlock = document.createElement('div');
//                         rightBlock.style.cssText = `position: absolute !important; top: 0 !important; right: 0 !important; width: 0% !important; height: 100% !important; background-color: #000000 !important;`;
...
//                         let target = document.body || document.documentElement;
//                         if (target) target.appendChild(container);
//                         console.log('[⬛] Dynamic Black Overlay Injected Successfully (Top/Bottom: 20%, Left/Right: 0%).');
//                     }
//                 } catch(e) {}
//             }, 1000); 
//         });
...
//             for (const frame of page.frames()) {
//                 try {
//                     // 🔥 BUGS FIXED HERE: Pehle yeh zabardasti unmute kar raha tha, ab 'startMuted' flag ko use karega
//                     const autoPlayed = await frame.evaluate((isMuted) => {
//                         let playing = false;
//                         document.querySelectorAll('video').forEach(v => {
//                             if (v.clientWidth > 50 && !v.paused && v.currentTime > 0) {
...
//                     // 🔥 Strictly applying the mute setting passed down
//                     mediaElements.forEach(media => { 
//                         media.muted = muteVideo; 
//                         media.volume = muteVideo ? 0.0 : 1.0; 
//                     });
...
//                 // 🔥 BUGS FIXED HERE: Make sure Swap Refresh applies exact audio setting URL properly
//                 let urlToInit = isProactiveRefresh ? activeUrlStr : backupUrlStr;
//                 await initializeVideo(backupPage, !ENABLE_STREAM_AUDIO, true, urlToInit); 
...
// =============== done bs upper yeh black overlay ko kaam karna chta hu test k liye left and right 0% and top and bottom 20% ===============
...
//                         let target = document.body || document.documentElement;
//                         if (target) target.appendChild(container);
//                         console.log('[⬛] Dynamic Black Overlay Injected Successfully.');
//                     }
//                 } catch(e) {}
//             }, 1000); 
//         });
...
//                     // 🔥 Applied Dynamic Stream Mute Control
//                     mediaElements.forEach(media => { 
//                         media.muted = muteVideo; 
//                         media.volume = muteVideo ? 0.0 : 1.0; 
//                     });
...
//                 // 🔥 Pass Dynamic Mute Status to Swap Initialization
//                 await initializeVideo(backupPage, !ENABLE_STREAM_AUDIO, true, backupUrlStr); 
...
//     // 🔥 Initializing with strictly enforced mute status
//     await initializeVideo(activePage, !ENABLE_STREAM_AUDIO, true, urlList[currentUrlIndex]); 
...
// ============== gemeini see khlty see audio804 ko control keya, upper pher see iframe video ko control  =====================

// # ================ iss below waley  me hum audio ko control karey manullay on and off ================
...
// const selectedFormat = process.env.STREAM_FORMAT || 'Original (16:9 Standard)'; 
// const ENABLE_BLACK_OVERLAY = process.env.ENABLE_BLACK_OVERLAY === 'ON';
// const ENABLE_BACKGROUND_AUDIO = process.env.ENABLE_BACKGROUND_AUDIO === 'ON'; // 🔥 AUDIO SWITCH
...
//     // =========================================================================================
//     // 🔊 BACKGROUND AUDIO INJECTION (INFINITE LOOP) - CONTROLLED VIA WORKFLOW
//     // =========================================================================================
//     if (ENABLE_BACKGROUND_AUDIO) {
//         const possibleAudioExts = ['.mp3', '.wav', '.m4a', '.aac', '.mp4'];
//         let foundAudioPath = null;
...
//             const cmd = `gh workflow run main.yml -f target_urls="${targetUrls}" -f youtube_stream_key="${YT_KEY}" -f facebook_stream_key="${FB_KEY}" -f stream_format="${format}" -f stream_quality="${quality}" -f server_selection="${server}" -f proxy_engine="${PROXY_ENGINE}" -f enable_black_overlay="${blackOverlayStatus}" -f enable_background_audio="${bgAudioStatus}" -f custom_duration="None"`;
//             execSync(cmd, { stdio: 'inherit' });
//             setTimeout(async () => {
//                 await cleanup(); 
//                 process.exit(0); 
...
// ====================== new black overlay iss below waleyu me hai ===============================
...
// const selectedQuality = process.env.STREAM_QUALITY || 'Original (1080p Max)';
// const selectedFormat = process.env.STREAM_FORMAT || 'Original (16:9 Standard)'; 
// const ENABLE_BLACK_OVERLAY = process.env.ENABLE_BLACK_OVERLAY === 'ON'; // 🔥 OVERLAY SWITCH
...
//                         const container = document.createElement('div');
//                         container.id = 'sport4u-black-overlay';
//                         // z-index 2147483646 ensures it's above video but below text watermark
//                         container.style.cssText = `
//                             position: fixed !important; top: 0 !important; left: 0 !important;
//                             width: 100vw !important; height: 100vh !important;
//                             pointer-events: none !important; z-index: 2147483646 !important;
...
//                         mainIframe.style.setProperty('width', '100vw', 'important');
//                         mainIframe.style.setProperty('height', '100vh', 'important');
//                         // 🔥 Video Layer below Both Overlays (Black Overlay is 2147483646)
//                         mainIframe.style.setProperty('z-index', '2147483645', 'important'); 
//                         mainIframe.style.setProperty('background-color', 'black', 'important');
//                         mainIframe.style.setProperty('border', 'none', 'important');
//                         mainIframe.style.setProperty('opacity', '1', 'important');
...
//                     // 🔥 Modified Junk remover so it won't touch our Black Overlay
//                     const junkClasses = '.chat, #chat, header, footer, .sidebar, .banner, .ads, [class*="overlay"]:not(#smart-stream-overlay):not(#sport4u-watermark):not(#sport4u-black-overlay), [id*="pop"], [class*="pop"], a[href*="extension"], [class*="notification"], [id*="notification"]';
//                     document.querySelectorAll(junkClasses).forEach(el => { 
//                         try { el.remove(); } catch(e){ el.style.setProperty('display', 'none', 'important'); } 
//                     });
...
//                     const adKeywords = ['jerk', 'mate', 'free', 'online', 'adult', 'dating', 'close', 'notification', 'justine', 'paying', 'job'];
//                     document.querySelectorAll('div, section, span, a').forEach(el => {
//                         // 🔥 PROTECT WATERMARK & BLACK OVERLAY
//                         if (el.id === 'smart-stream-overlay' || el.id === 'sport4u-watermark' || el.id === 'sport4u-black-overlay') return;
...
//                                 width: 100vw !important; height: 100vh !important;
//                                 max-width: 100vw !important; max-height: 100vh !important;
//                                 z-index: 2147483645 !important; /* 🔥 Video Layer */
//                                 background-color: #000000 !important;
//                                 object-fit: contain !important; transform: none !important; margin: 0 !important; padding: 0 !important;
//                             }
//                             ytd-player, #ytd-player, .html5-video-container, .html5-video-player,
...
//     // 🔥 Inject OVERLAYS on initial load
//     await injectBlackOverlay(activePage);
//     await injectOfficialWatermark(activePage);
//     await hideLoadingUI(activePage); 
...
//             const server = process.env.SERVER_SELECTION || 'None';
//             const format = process.env.STREAM_FORMAT || 'Original (16:9 Standard)'; 
//             const blackOverlayStatus = process.env.ENABLE_BLACK_OVERLAY || 'OFF'; // 🔥 Added state persistence
            
//             const cmd = `gh workflow run main.yml -f target_urls="${targetUrls}" -f youtube_stream_key="${YT_KEY}" -f facebook_stream_key="${FB_KEY}" -f stream_format="${format}" -f stream_quality="${quality}" -f server_selection="${server}" -f proxy_engine="${PROXY_ENGINE}" -f enable_black_overlay="${blackOverlayStatus}" -f custom_duration="None"`;
//             execSync(cmd, { stdio: 'inherit' });
//             setTimeout(async () => {
//                 await cleanup(); 
//                 process.exit(0); 
...
// =========== yeh below iss me audio add karty hai . Grear audio done aab upper waley mee eek new overlay add karty hai black ovverlay jukey live match ko hide karey for facebook live ============================
...
// let browser = null;
// let obsProcess = null;
// let audioProcess = null; // 🔊 Audio Process Loop ke liye add kiya
// let activePage = null;
// let backupPage = null;
...
//     // =========================================================================================
//     // 🔊 BACKGROUND AUDIO INJECTION (INFINITE LOOP)
//     // =========================================================================================
//     // const possibleAudioExts = ['.mp3', '.wav', '.m4a', '.aac'];
//   // Update this specific line in your stream.js file:
// const possibleAudioExts = ['.mp3', '.wav', '.m4a', '.aac', '.mp4'];
//     let foundAudioPath = null;
//     for (let ext of possibleAudioExts) {
//         let tempPath = path.join(process.cwd(), `audio804${ext}`);
...
//         console.log(`[🎶] Background Audio found: ${foundAudioPath}`);
//         console.log(`[🎶] Starting Audio in INFINITE LOOP...`);
//         // ffplay command audio ko headless loop mein chalaye gi 
//         audioProcess = spawn('ffplay', ['-nodisp', '-loop', '0', '-loglevel', 'quiet', foundAudioPath]);
//         audioProcess.on('error', (err) => console.log(`[!] Audio Process Error: ${err.message}`));
//     } else {
//         console.log(`[!] Audio file (audio804.mp3 ya .wav) NOT FOUND in main folder. Skipping background audio.`);
//     }
//     // =========================================================================================
...
//     if (browser) { try { await browser.close(); } catch(e) { } browser = null; }
//     if (obsProcess) { try { obsProcess.kill('SIGKILL'); } catch(e) { } obsProcess = null; }
//     if (audioProcess) { try { audioProcess.kill('SIGKILL'); } catch(e) { } audioProcess = null; } // 🔊 Audio kill handler
//     try {
//         execSync('pkill -9 obs || true', { stdio: 'ignore' });
//         execSync('pkill -9 chrome || true', { stdio: 'ignore' });
//         execSync('pkill -9 puppeteer || true', { stdio: 'ignore' });
//         execSync('pkill -9 ffplay || true', { stdio: 'ignore' }); // 🔊 Force kill ffplay engine
//     } catch (e) { }
// }
...
// ===================== ALhamdullah done bas upper yeh new extension add karty hai "Allow cors:access content ===========
...
// iss me issue hai k facebook par live nahy hu raaha hai . issme srf url par jawo and broadcast
...
// else if (STREAM_QUALITY === '1080p') { RES_W = 1920; RES_H = 1080; BITRATE = 4500; }
...
//         console.log(`[🚀] TARGET PLATFORM: FACEBOOK`);
//     } else {
//         console.log(`[❌] ERROR: Kam az kam ek Stream Key daalna zaroori hai!`);
//         process.exit(1);
//     }
...
// async function main() {
//     console.log('\n==================================================');
//     console.log('     STREAM CAPTURE ENGINE STARTED (ULTRAFAST)');
//     console.log('==================================================\n');
...
// =================== live facebook youtube full latest video fullscreen youtube shorts . upper latest ha ==================
