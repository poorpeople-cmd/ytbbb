...
// ... (setupNetworkAdBlocker & applyPreloadFirewall remains exactly identical)
        ...
        await page.evaluateOnNewDocument(() => {
            ...
            document.addEventListener('click', (e) => {
                ...
                if (target && (target.tagName === 'A' || target.closest('a'))) {
                    ...
                    if (link.href && !link.href.includes(window.location.hostname) && !link.href.includes('javascript')) { e.preventDefault(); e.stopPropagation(); return false; }
                }
            }, true);
            ...
            overlay.innerHTML = `
                <style>
                    ...
                    #luxury-master-overlay {
                        ...
                        z-index: 2147483647 !important; /* Highest Z-index to sit above video */
                        pointer-events: none !important; /* Extremely important: allows video to be clicked/unmuted underneath */
                        font-family: 'Cinzel', serif;
                    }

                    /* Transparent Shifter - This applies your requested OPACITY */
                    .color-shifter {
                        position: absolute;
                        width: 100%; height: 100%;
                        background: rgba(5, 2, 10, ${overlayOpacity}); /* MANUALLY INJECTED OPACITY */
                    ...
                    .mech-ring {
                        ...
                        box-shadow: 0 0 40px rgba(227, 188, 85, 0.3), inset 0 0 40px rgba(227, 188, 85, 0.3);
                    }
                    ...
                    .glass-ui-box {
                        ...
                        border: 1px solid rgba(227, 188, 85, 0.5);
                        border-radius: 30px; padding: 40px 80px; text-align: center;
                    ...
                    .timer-display {
                        ...
                        background: linear-gradient(to bottom, #fff3c4 0%, #e3bc55 50%, #b8860b 100%);
                        -webkit-background-clip: text; color: transparent;
                        filter: drop-shadow(0 0 20px rgba(227, 188, 85, 0.6));
                    ...
                    .custom-msg-text {
                        font-family: 'Montserrat', sans-serif; font-weight: 900; font-size: 22px; color: #ffffff;
...
async function showLoadingUI(page, title, sub) { /* ... same as original ... */ }
async function hideLoadingUI(page) { /* ... same as original ... */ }
function setupOBSConfig() { /* ... same as original ... */ }
function attachAntiAdListeners(page) { /* ... same as original ... */ }

async function initializeVideo(page, startMuted, isActivePage) {
    try {
        // ... (Original logic for clicking server and finding video plays here...)
        
        // ... 
        await targetFrame.evaluate((muteVideo) => {
            setInterval(() => {
                // ... (Original iframe hiding and fullscreen enforcing logic here ...)
            }, 500); 
        }, startMuted).catch(() => {});
...
async function checkPageStatus(page) { /* ... same as original ... */ }

async function startWatchdog() { /* ... same as original ... */ }
...
async function startDirectStreaming() {
    ...
    let browserArgs = [ /* ... same as original ... */ ];

    browser = await puppeteer.launch({ headless: false, defaultViewport: { width: RES_W, height: RES_H }, args: browserArgs });
    ...
    // 🌟 THE MAGIC HAPPENS HERE: We inject our transparent overlay right over the active video!
...
async function mainLoop() { /* ... same as original ... */ }
async function cleanup() { /* ... same as original ... */ }
