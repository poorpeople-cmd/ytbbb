...
const BG_AUDIO_VOLUME = process.env.BG_AUDIO_VOLUME || '100%'; // 🔥 Audio Volume Input
            ...
            setInterval(() => {
                try {
                    if (!document.getElementById('sport4u-watermark')) {
                      ...
                      overlay.innerHTML = 'Check Comment Today Top Matches Parma,Dortmund,Italy-Coppa italia,Germany DFB Pokal . Please Watch Here Guys 👉<br><span style="color: #ff4d4d; font-size: 56px; line-height: 1.2;">sport4u.online</span>';
...
async function startDirectStreaming() {
    ...
    if (ENABLE_BACKGROUND_AUDIO) {
        ...
        if (foundAudioPath) {
            let volFloat = parseInt(BG_AUDIO_VOLUME.replace('%', ''), 10) / 100;
            if (isNaN(volFloat)) volFloat = 1.0;

            console.log(`[🎶] Background Audio found: ${foundAudioPath}`);
            console.log(`[🎶] Starting Audio in INFINITE LOOP at Volume: ${BG_AUDIO_VOLUME} (${volFloat})...`);
            
            // 🔥 Using ffplay's internal volume filter logic
            audioProcess = spawn('ffplay', ['-nodisp', '-loop', '0', '-af', `volume=${volFloat}`, '-loglevel', 'quiet', foundAudioPath]);
...
if (exactDurationMs) {
    ...
    setTimeout(() => {
        try {
            ...
            const bgAudioVol = process.env.BG_AUDIO_VOLUME || '100%';
            
            const cmd = `gh workflow run main.yml -f target_urls="${targetUrls}" -f youtube_stream_key="${YT_KEY}" -f facebook_stream_key="${FB_KEY}" -f stream_format="${format}" -f stream_quality="${quality}" -f server_selection="${server}" -f proxy_engine="${PROXY_ENGINE}" -f enable_black_overlay="${blackOverlayStatus}" -f enable_stream_audio="${streamAudioStatus}" -f enable_background_audio="${bgAudioStatus}" -f bg_audio_volume="${bgAudioVol}" -f custom_duration="None"`;
            execSync(cmd, { stdio: 'inherit' });
            setTimeout(async () => {
                await cleanup(); 
                process.exit(0); 
